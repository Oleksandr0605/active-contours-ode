...
Недолік: ці оператори "бачать" лише найближчі пікселі. Вони не мають поняття про "контур" як цілісний об'єкт. В результаті роботи детектора ми часто отримуємо набір розрізнених пікселів, а не замкнену лінію. Високочастотний шум створює фальшиві градієнти, які локальні методи помилково сприймають як краї.
...
## Аналіз збіжності та енергії
### Аналітичне дослідження

Для строгого доведення збіжності запропонованої динамічної системи скористаємося прямим методом Ляпунова. 

Розглянемо повну енергію системи як функцію-кандидата Ляпунова $E(\mathbf{V}): \mathbb{R}^{2N} \to \mathbb{R}$:

$$
E(\mathbf{V}) = \frac{1}{2}\mathbf{V}^T A \mathbf{V} - \sum_{i=1}^N P(\mathbf{v}_i)
$$

де перший доданок — це квадратична форма внутрішньої енергії (визначена додатно, оскільки матриця жорсткості $A$ є додатно визначеною), а другий — потенціальна енергія зовнішнього поля. Функція $E(\mathbf{V})$ обмежена знизу, оскільки інтенсивність зображення скінченна, а квадратична форма $E_{int} \ge 0$.

Знайдемо повну похідну енергії за часом вздовж траєкторії системи. За правилом ланцюга:

$$
\frac{d}{dt} E(\mathbf{V}(t)) = \nabla_{\mathbf{V}} E(\mathbf{V}) \cdot \frac{d\mathbf{V}}{dt}
$$

З рівняння руху системи (градієнтного спуску) відомо, що швидкість зміни стану пропорційна від'ємному градієнту енергії:

$$
\frac{d\mathbf{V}}{dt} = -\nabla_{\mathbf{V}} E(\mathbf{V})
$$

Підставивши вираз швидкості у рівняння повної похідної, отримуємо:

$$
\frac{dE}{dt} = \nabla_{\mathbf{V}} E \cdot (-\nabla_{\mathbf{V}} E) = - \|\nabla_{\mathbf{V}} E\|^2 \le 0
$$

**Висновки з аналізу:**
1.  **Монотонність:** Похідна енергії за часом є недодатною ($\frac{dE}{dt} \le 0$) для будь-якого стану системи. Це означає, що енергія системи монотонно спадає в процесі еволюції.
2.  **Умова рівноваги:** Зміна енергії припиняється ($\frac{dE}{dt} = 0$) тоді і тільки тоді, коли $\|\nabla_{\mathbf{V}} E\| = 0$. Це відповідає стаціонарній точці, де сума внутрішніх та зовнішніх сил дорівнює нулю.
3.  **Асимптотична стійкість:** Оскільки енергія обмежена знизу і монотонно спадає, траєкторія $\mathbf{V}(t)$ асимптотично збігається до множини локальних мінімумів енергетичного функціоналу (теорема Барбашина-Красовського).

### Експерементальні результати

Ключовим критерієм стійкості динамічної системи є поведінка її повної енергії $E_{total}(t)$. 
![alt text](energy_convergence_plot.png)
Рис. 1. Еволюція повної енергії активного контуру.  

На рис. 1 наведено графік залежності енергії від часу ітерації для реалізованої моделі. Синя лінія — поточна енергія, червоний пунктир — досягнутий мінімум. Як видно з графіка , енергія системи монотонно спадає від початкового значення до локального мінімуму (286), що відповідає стану рівноваги контуру на межах об'єкта. Відсутність осциляцій енергії підтверджує коректність вибору кроку дискретизації часу $\gamma$ та стійкість напівнеявної чисельної схеми. Така поведінка енергії дозволяє трактувати її як функцію Ляпунова для даної динамічної системи, гарантуючи асимптотичну стійкість розв'язку.
